...
def test_summarize_and_forecast_e2e(temp_db, sample_article, sample_macro, mock_summary, mock_forecast):
    with patch("src.backend.services.summarizer.GeminiClient") as summarize_client:
    ...
    with patch("src.backend.services.predictor.GeminiClient") as forecast_client:
...
def test_full_pipeline_skip_scrape_e2e(
    ...
    with (
        patch("src.backend.services.summarizer.GeminiClient") as summarize_client,
        patch("src.backend.services.predictor.GeminiClient") as forecast_client,
    ):
        summarize_client.return_value.generate_json.return_value = mock_summary
        forecast_client.return_value.generate_json.return_value = mock_forecast
